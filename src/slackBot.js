...
async function symbolsStatus(userText, say) {
  for (const url of listOfSymbols) {
    ...
    let {
      order_book_lastUpdated,
      candlestick_lastUpdated,
      ticker_lastUpdated,
      last_price,
      ...
      if (userText.includes(" ")) {
        ...
        switch (userTextFirstIndex) {
          ...
          default: {
            if (message?.user) {
              await say(
                `*${userText}* - please check the command name (or) enter *help* to see the list of commands`
              );
            }
            break;
          }
...
async function helpCommand(say) {
  await say(`List of commands:\n
    ...
    *kucoin BTC-USDT* - To check symbol status in Kraken exchange\n
...
async function addSymbolsToDisconnect(userText, say, message) {
  ...
  (await getSymbols()).forEach(async (value) => {
    ...
    if (userText.split("-")[1].toLowerCase() === symbol.toLowerCase()) {
      ...
      disconnectedUser.set(
        userText.split("-")[1].toUpperCase(),
        userInfo?.user?.real_name.toUpperCase()
      );
      await say(
        `*${userText.split("-")[1].toUpperCase()}* disconnected manually...`
      );
...
async function deleteSmbolsDisconnectMap(userText, say) {
  if (disconnectSymbol.has(userText.split("+")[1].toLowerCase())) {
    ...
    await say(
      `*${userText.split("+")[1].toUpperCase()}* connected manually...`
    );
    manuallyDisconnected.set(userText.split("+")[1].toLowerCase(), {
      disconnected: true,
    });
    ...
    await say(
      `${userText
        .split("+")[1]
        .toUpperCase()}, This symbol not add to *Disconnect*\nTo add use this command *"-${userText
        .split("+")[1]
...
async function deletePrioritySymbols(userText, say) {
  if (prioritySymbols.has(userText.split(" ")[1].toLowerCase())) {
    ...
    await say(
      `${userText
        .split(" ")[1]
        .toUpperCase()}, This symbol not add to *Priority*\nTo add use this command *"add ${userText
        .split(" ")[1]
        .toUpperCase()}"* or use the *"help"* command to see the list of commands`
    );
  }
...
async function addSymbols_pm2(userText, say) {
  (await getSymbols()).forEach(async (value) => {
    ...
    if (symbol.toLowerCase() === userText.split(" ")[1]) {
      ...
      pm2SymbolStatus.set(userText.split(" ")[1].toLowerCase(), {
        status: false,
      });
...
async function deleteSymbols_pm2(userText, say) {
  if (pm2Symbol.has(userText.split(" ")[1].toLowerCase())) {
    ...
    await say(
      `${userText
        .split(" ")[1]
        .toUpperCase()}, This symbol not add to *pm2 list*\nTo add use this command *"add-pm2 ${userText
        .split(" ")[1]
        .toUpperCase()}"* or use the *"help"* command to see the list of commands`
    );
  }
...
async function validateBinanceSymbols(userText, say, messageStatus = true) {
  ...
  if (symbolName.includes("-")) {
    await say(
      `${symbolName} - please enter symbol name as given example => *BTCUSDT* for binance exchange`
    );
    return;
  }
  try {
    ...
    if (response.data?.symbols[0]?.symbol === symbolName && messageStatus) {
      await say(
        `Binance symbol status: *${symbolName} - ${response.data?.symbols[0]?.status}*`
      );
    } else if (
      response.data?.symbols[0]?.symbol === symbolName &&
      !messageStatus &&
      response.data?.symbols[0]?.status !== "TRADING"
    ) {
      other_exchange = true;
    ...
    if (err?.response?.data?.msg === "Invalid symbol.") {
      await say(
        `*${symbolName}* - symbol not found in binance exchange (or) please check the symbol name`
      );
    } else {
      other_exchange = true;
      console.error(
        `Error fetching Binance status for symbol ${symbolName}`,
        err?.response?.data
      );
      await say(
        `Error fetching ${symbolName} symbol from Binance exchange, please try again...`
      );
    }
  }
}
...
async function validateMexcSymbols(userText, say, messageStatus = true) {
  ...
  if (symbolName.includes("-")) {
    await say(
      `${symbolName} - please enter symbol name as given example => *BTCUSDT* for MECX exchange`
    );
    return;
  }
  try {
    ...
    if (data?.symbols[0]?.symbol === symbolName && messageStatus) {
      await say(
        `*${symbolName}* spot trading status: ${data?.symbols[0]?.isSpotTradingAllowed}`
      );
    } else if (
      data?.symbols[0]?.symbol === symbolName &&
      !messageStatus &&
      !data?.symbols[0]?.isSpotTradingAllowed
    ) {
      other_exchange = true;
      other_exchange_status.set(symbolName, {
        status: false,
      ...
      await say(
        `*${symbolName}* symbol not present in mexc exchange (or) please check the symbol name`
      );
    }
  } catch (err) {
    other_exchange = true;
    await say(
      `Error fetching ${symbolName} symbol from MEXC exchange, please try again...`
    );
  }
}
...
async function validate_okx_symbols(userText, say, messageStatus = true) {
  ...
  if (!symbolName.includes("-")) {
    await say(
      `${symbolName} - please enter symbol name as given example => *BTC-USDT* for OKX exchange`
    );
    return;
  }
  ...
  try {
    ...
    if (data?.data[0]?.instId === symbolName && messageStatus) {
      await say(
        `${data?.data[0]?.baseCcy}${data.data[0].quoteCcy} - OKX symbol status: ${data?.data[0]?.state}`
      );
    } else if (
      data?.data[0]?.instId === symbolName &&
      !messageStatus &&
      data?.data[0]?.state !== "live"
    ) {
      other_exchange = true;
      other_exchange_status.set(symbolName, {
        status: false,
      ...
      await say(
        `${data?.data[0]?.baseCcy}${data.data[0].quoteCcy} - symbol not present in OKX (or) please check the symbol name eg:(BTC-USDT)`
      );
    }
  } catch (err) {
    other_exchange = true;
    await say(
      `Error fetching ${symbolName} symbol from OKX exchange, please try again...`
    );
    console.error(
      "❌ OKX exchange API error:",
...
async function validate_kraken_symbols(userText, say, messageStatus = true) {
  ...
  if (symbolName.includes("-")) {
    await say(
      `${symbolName} - please enter symbol name as given example => *BTCUSDT* for kraken exchange`
    );
    return;
  }
  try {
    ...
    if (data?.result) {
      for (const [key, value] of Object.entries(data?.result)) {
        if (
          (key === symbolName || value?.altname === symbolName) &&
          messageStatus
        ) {
          ...
          await say(
            `${symbolName} - symbol not present in Kraken exchange (or) please check the symbol name`
          );
        }
      }
    } else {
    ...
    other_exchange = true;
    await say(
      `Error fetching ${symbolName} symbol from Kraken exchange, please try again...`
    );
  }
}
...
async function validate_kucoin_symbol(userText, say, messageStatus = true) {
  ...
  if (!symbolName.includes("-")) {
    await say(
      `${symbolName} - please enter symbol name as given example => *BTC-USDT*, for kucoin exchange`
    );
    return;
  }
  try {
    ...
    if (data?.data?.symbol === symbolName && messageStatus) {
      await say(
        `Kucoin *${symbolName}* symbol status: *${data?.data?.enableTrading}*`
      );
    } else if (
      data?.data?.symbol === symbolName &&
      !messageStatus &&
      !data?.data?.enableTrading
    ) {
      other_exchange = true;
      other_exchange_status.set(symbolName, {
        status: false,
      ...
      await say(
        `${symbolName} - symbol not present in kucoin exchange (or) please check the name eg:(BTC-USDT)`
      );
    }
  } catch (error) {
    other_exchange = true;
    await say(
      `Error fetching ${symbolName} symbol from Kucoin exchange, please try again...`
    );
  }
}
  ...
  try {
    if (!other_exchange) {
      ...
      for (const [key, value] of other_exchange_status) {
        results =
          results +
          `${value?.exchange_name} *${key}* symbol status: *${value?.exchange_staus}*\n`;
      }
      await say(results);
